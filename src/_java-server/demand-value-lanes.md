...
A **demand lane** does not store any value, instead they are backed by a [value lane]({% link _java-server/value-lanes.md %}) and retrieve values purely on demand. Demand lanes **lazily** generate events for the value lane, only when there is an uplink ready to receive the event. The lane meets these requirements: 
