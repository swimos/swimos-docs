...
Either way, the process that talks directly to the Swim server updates the server by either [sending commands]({% link _java-server/command-lanes.md %}) or [writing to downlinks]({% link _java-server/downlinks.md %}).
