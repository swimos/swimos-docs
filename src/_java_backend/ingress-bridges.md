...
redirect_from:
  - /java/tutorials/ingress-bridges/
  - /java/reference/ingress-bridges.html
---
...
Either way, the process that talks directly to the Swim server updates the server by either [sending commands]({% link _java_backend/command-lanes.md %}) or [writing to downlinks]({% link _java_backend/downlinks.md %}).
