...
redirect_from:
  - /java/guides/introspection.html
---
...
This guide uses the [`swim-cli`]({% link _java_backend/cli.md %}) extensively._
...
Notice the room nodes have two agents, a `RoomAgent` and `RoomSimulatorAgent` - see more on multiple agents in the [`traits`]({% link _java_backend/traits.md %}) reference.
