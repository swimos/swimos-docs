...
Time series data is a perfect candidate for a Swim [`MapLane`]({% link _java_backend/map-lanes.md %}).
...
(built-ins trivially, and custom types by using [`Forms`]({% link _java_backend/forms.md %})).
...
Next we are going to create and manage a [`Timer`]({% link _java_backend/timers.md %}) that will handle the calling of the `trimHistory()` method we just created.
