...
A **demand map lane** does not store a collection, instead they are backed by a [map lane]({% link _backend/map-lanes.md %}) and retrieve values purely on demand. Demand map lanes **lazily** generate events for the map lane, only when there is an uplink ready to receive the event. The lane meets these requirements:
