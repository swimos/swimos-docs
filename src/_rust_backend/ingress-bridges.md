...
redirect_from:
  - /rust/tutorials/ingress-bridges/
  - /rust/reference/ingress-bridges.html
---
...
Either way, the process that talks directly to the Swim server updates the server by either [sending commands]({% link _rust_backend/command-lanes.md %}) or [writing to downlinks]({% link _rust_backend/downlinks.md %}).
