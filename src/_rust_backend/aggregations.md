...
Let's take a simple vehicle agent as a starting point, the main component of which is a [`ValueLane`]({% link _rust_backend/value-lanes.md %}) that stores the vehicle's status.
...
A [`JoinValueLane`]({% link _rust_backend/join-value-lanes.md %}) inside the `StateAgent` will become a map, the key being some unique identifier of a vehicle, and the value being the vehicle's current status.
The lane will keep itself up to date with the `status` of the vehicle through the use of [`downlinks`]({% link _rust_backend/downlinks.md %}).
...
To do this we open a [`CommandLane`]({% link _rust_backend/command-lanes.md %}) named `addVehicle` accepting a `Uri` - which will be the vehicle's node URI.
...
_Note: We have to convert the vehicle's node URI to a `Value` to send it via a command. For more information on this see [Forms]({% link _rust_backend/forms.md %})._
