...
redirect_from:
  - /rust/guides/summary-statistics.html
---
...
Running [offline algorithms](https://en.wikipedia.org/wiki/Online_algorithm) against a [time series]({% link _rust_backend/time-series.md %}) that contains every received message can compute _any_ (solvable) statistic of interest, but this approach incurs linearly increasing memory and time costs.
...
_Note: While we discourage reliance on it for computations, we could easily attach a [time series]({% link _rust_backend/time-series.md %}) of message history to `TowerAgent` (or its variations described in the upcoming sections)._
...
The following [`swim-cli`]({% link _rust_backend/cli.md %}) commands are available while the process runs (replace `$ID` instances with either `2350` or `2171`):
